...
Vue.component('note', {
	props: ['note'],
	template: '<div class="note"><textarea>{{ note.message }}</textarea></div>'
});
...
function vueInit() {
	var app = new Vue({
		...
		data: {
			notes: [
				// { message: "this is a test", noteOrder: 0, active: true, style: getNoteStyles(0) },
				// { message: "here's another note", noteOrder: 1, active: false, style: getNoteStyles(1) }
			]
		},
		mounted: function() {
			// this.notes.push({ message: "this is a test", noteOrder: 0, active: true, style: this.getNoteStyles(0) });
		},
		computed: {
			hasNotes: function() {
				return this.notes.length > 0;
			},
			getNoteStyles: function(order) {
				return {
					left: order * 10,
					top: order * 10,
					zIndex: order
